...
class Login extends Component {
    ...
    //注册
    zuce(){
        if (!/^1[3-9]\d{9}$/i.test(this.state.username)) {
            alert("你的手机号不满足条件");
            return false;
        };
        if (!/^[^\s]{6,19}$/.test(this.state.password)) {
            alert("密码有误");
            return false;
        };
        ...
        $.ajax({
            ...
            url: "http://localhost:3001/login/zuce",
            async: false,
                    ...
                    <div className="wrap loginPage">
                        <div id="username_login" className="login-wrap login-wrap-active">
                            <div className="input-container">
                                <input id="username" onChange={this.user.bind(this)} className="acc-input username txt-input J_ping" type="text" placeholder="请用手机号码注册" />
                                <i className="icon icon-clear"></i>
                            </div>
                            <div className="input-container">
                                <input id="password" onChange={this.pass.bind(this)} type="password" className="acc-input password txt-input J_ping" placeholder="请输入密码/6-18位" />
                                <i className="icon icon-clear"></i>
                            </div>
                            <div className="input-container">
                                <input onChange={this.pay.bind(this)} type="password" className="acc-input password txt-input J_ping" placeholder="支付码/6位数字" />
                                <i className="icon icon-clear"></i>
                            </div>
                        </div>
                        <div className="notice">&nbsp;</div>
                        <a  onClick={this.zuce.bind(this)} id="loginBtn" className="btn J_ping" report-eventid="MLoginRegister_Login">注册</a>
